...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    /* cdm (UNIFIED SATURATION UNIVERSE: QUANTUM FLUID) */

    class_define_index(ppv->index_pt_delta_cdm,1,index_pt,1); /* fluid density */
    
    // The quantum fluid has pressure and viscosity, meaning it flows differently than 
    // the metric expansion. We MUST allocate memory for its velocity in ALL gauges!
    class_define_index(ppv->index_pt_theta_cdm,1,index_pt,1); /* fluid velocity */
...
int perturbations_total_stress_energy(
  ...
  /** - define local variables */
   double rhoqf = ppw->pvecback[pba->index_bg_rho_qf];
  double wqf   = ppw->pvecback[pba->index_bg_w_qf];
  double cs2qf = ppw->pvecback[pba->index_bg_cs2_qf];

  /* 2. Retrieve perturbations (reusing the CDM slots) */
  /* Note: Check your background.h/perturbations.h if you spelled it 
     index_pt_delta_cdm (standard) or index_pt_deltacdm (modified) */
  double deltqf  = y[ppw->pv->index_pt_delta_cdm]; 
  double thetaqf = y[ppw->pv->index_pt_theta_cdm];

  /* 3. Add to the total stress-energy tensor */
  /* Density perturbation: δρ */
  ppw->delta_rho += rhoqf * deltqf;

  /* Velocity/Momentum perturbation: (ρ+P)θ */
  ppw->rho_plus_p_theta += (rhoqf + wqf * rhoqf) * thetaqf;

  /* Pressure perturbation: δP = c_s^2 * δρ */
  ppw->delta_p += cs2qf * rhoqf * deltqf;
  ...
  if (_scalars_) {
    ...
    if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
      ...
      /* unified quantum fluid contribution (reusing CDM slots) */

        /* === UNIFIED SATURATION UNIVERSE: Quantum fluid contribution === */
    /* REMOVED if (pba->has_cdm == _TRUE_) check so it runs even if Omega_cdm=0 */
    
    double rho_qf, w_qf, cs2_qf;
    double delta_qf, theta_qf;
    double delta_rho_qf, delta_p_qf;
    double rho_plus_p_qf;

    /* 1. Retrieve background quantities */
    rho_qf = ppw->pvecback[pba->index_bg_rho_qf];
    w_qf   = ppw->pvecback[pba->index_bg_w_qf];
    cs2_qf = ppw->pvecback[pba->index_bg_cs2_qf];

    /* 2. Retrieve perturbations from reused CDM slots */
    /* Note: Ensure background.c/perturbations.c allocates these indices even if has_cdm is false */
    delta_qf = y[ppw->pv->index_pt_delta_cdm];

    if (ppt->gauge == newtonian) {
      theta_qf = y[ppw->pv->index_pt_theta_cdm];
    }
    else {
      /* Synchronous gauge convention: dark matter velocity is zero (defines the frame) */
      /* BUT for a viscous fluid, theta is non-zero. If you treat it strictly as 
         CDM-like in synchronous, keep 0. If it flows, use the variable. 
         For now, keeping 0 preserves the 'CDM reuse' logic, but for viscosity 
         you eventually want theta_qf here too. */
      theta_qf = 0.; 
    }

    /* 3. Compute fluid perturbations */
    delta_rho_qf      = rho_qf * delta_qf;
    delta_p_qf        = cs2_qf * delta_rho_qf;       /* Adiabatic pressure perturbation */
    rho_plus_p_qf     = (1. + w_qf) * rho_qf;

    /* 4. ADD TO TOTALS (The Einstein Coupling) */
    ppw->delta_rho       += delta_rho_qf;
    ppw->delta_p         += delta_p_qf;
    ppw->rho_plus_p_tot  += rho_plus_p_qf;

    if (ppt->gauge == newtonian) {
       ppw->rho_plus_p_theta += rho_plus_p_qf * theta_qf;
    }

    /* 5. Register as "Matter" for P(k) output if requested */
    if (ppt->has_source_delta_m == _TRUE_) {
       delta_rho_m += delta_rho_qf;
       rho_m       += rho_qf;
    }

    if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
       if (ppt->gauge == newtonian) {
          rho_plus_p_theta_m += rho_plus_p_qf * theta_qf;
       }
       rho_plus_p_m += rho_plus_p_qf;
    }
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
   ...
   /** - ---> cdm (UNIFIED SATURATION UNIVERSE: VISCOUS QUANTUM FLUID) */
    if (pba->has_cdm == _TRUE_) {

      // Fetch the fluid parameters we calculated in background.c
      double w = pvecback[pba->index_bg_w_qf];     // Fluid Equation of State
      double cs2 = pvecback[pba->index_bg_cs2_qf]; // Adiabatic sound speed squared
      double nu_visc = pba->qf_viscosity;          // Shear viscosity parameter

      // Safety check: Prevent division by zero when the universe reaches Dark Energy limit (w = -1)
      double w_safe = (w < -0.999999) ? -0.999999 : w;

      // 1. The Continuity Equation (Density Evolution)
      // Math: \dot{\delta} = -(1+w)\theta - 3H(c_s^2 - w)\delta - 3(1+w)\dot{\phi}
      // Note: metric_continuity natively handles both newtonian and synchronous gauges
      dy[pv->index_pt_delta_cdm] = - (1.0 + w) * y[pv->index_pt_theta_cdm] 
                                   - 3.0 * a_prime_over_a * (cs2 - w) * y[pv->index_pt_delta_cdm]
                                   - 3.0 * (1.0 + w) * metric_continuity; 

      // 2. The Viscous Euler Equation (Velocity Evolution)
      // Math: \dot{\theta} = -H(1-3w)\theta + [k^2 c_s^2 / (1+w)]\delta - k^2\nu\theta + k^2\psi
      double hubble_friction = - a_prime_over_a * (1.0 - 3.0 * w) * y[pv->index_pt_theta_cdm];
      double internal_pressure = (k2 * cs2 / (1.0 + w_safe)) * y[pv->index_pt_delta_cdm];
      
      // metric_euler maps exactly to k2*psi in newtonian, and 0 in synchronous
      double gravity_pull = metric_euler;

      // THE SMOKING GUN: Viscous Friction Term
      double viscous_friction = - k2 * nu_visc * y[pv->index_pt_theta_cdm];

      // Sum them up to get the total change in fluid velocity
      dy[pv->index_pt_theta_cdm] = hubble_friction + internal_pressure + viscous_friction + gravity_pull;
    }
