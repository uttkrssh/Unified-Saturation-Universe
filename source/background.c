...
// UNIFIED SATURATION UNIVERSE: QUANTUM FLUID
// =========================================================================
double exponent = 3.0 * (1.0 + pba->qf_alpha);
double term_B = pba->qf_B * pow(a, -exponent);

// 1. Calculate the dimensionless density and pressure
double rho_qf_dim = pow((pba->qf_A + term_B), 1.0 / (1.0 + pba->qf_alpha));
double P_qf_dim = -pba->qf_A / pow(rho_qf_dim, pba->qf_alpha);

// 2. UNIT CORRECTION: Scale by the Hubble constant squared!
double rho_qf = rho_qf_dim * pow(pba->H0, 2);
double P_qf = P_qf_dim * pow(pba->H0, 2);

double w_qf = P_qf / rho_qf;

double cs2_qf = -pba->qf_alpha * w_qf;

// Store variables so perturbations.c can see them
pvecback[pba->index_bg_rho_qf] = rho_qf;
pvecback[pba->index_bg_w_qf]   = w_qf;
pvecback[pba->index_bg_cs2_qf] = cs2_qf;

// Add to the universe's total energy budget!
rho_tot += rho_qf;
p_tot += P_qf;
dp_dloga += cs2_qf * (-3.0) * (1.0 + w_qf) * rho_qf;
rho_m += rho_qf;
// =========================================================================
...
int background_indices(
  ...
  // =========================================================================
  // REGISTERING QUANTUM FLUID INDICES
  // =========================================================================
  class_define_index(pba->index_bg_rho_qf,_TRUE_,index_bg,1);
  class_define_index(pba->index_bg_w_qf,_TRUE_,index_bg,1);
  class_define_index(pba->index_bg_cs2_qf,_TRUE_,index_bg,1);
  // =========================================================================
